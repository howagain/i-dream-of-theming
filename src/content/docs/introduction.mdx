...
While rendering the page the browser parses the HTML and constructs a tree-like structured called the Document Object Model (DOM). As it encounters a stylesheet it shoots off a request to download it and then parses it.

When the browser starts parsing the CSS, it creates a set of rule sets, each one representing a group of selectors and properties. These rule sets are organized in a **tree-like structure, called the CSS Object Model (CSSOM), where each node represents a selector and each leaf node represents a property**. It is a parallel and analoguous data structure to the Document Object Model (DOM).

When the browswer is constructing this tree, it will pass down properties from the parent nodes to the child nodes. These are computed or inherited properties. 

The browser is also smart with media queries is will first check if the rule set applies to the current viewport. If it does, it will add the rule set to the CSSOM tree. If not, it will skip it. Additionally, the browser will ignore any rules that are not valid.

**Since CSS can be overwritten by later rules, if it finds a duplicate selector, it will overwrite the previous rule set with the new one.** This overwriting is why CSS is render blocking and why CSS variables are have different behavior from what you would expect in javascript. Additionally, 

After the full CSSOM is complete, browser then continues traversing the DOM, and for each element, it looks for the matching rule set in the CSSOM tree. When a match is found, the browser applies the properties to the element. If the element has a parent element, the browser also checks if the parent element has any inherited properties that should be applied to the element.

So, the browser uses a two-pass algorithm, where it first constructs the CSSOM tree and then matches the elements in the DOM with the rule sets in the CSSOM tree. The first pass is the construction of the CSSOM tree and the second pass is the matching and applying of the styles.

In other words, the browser first creates a map of all the CSS styles, and then it uses that map to apply the styles to the elements in the HTML. The browser uses inheritance to pass properties down to the selector nodes, then it traverses the tree to look for a match when it finds a node in the HTML.

#### What about CSS variables?

Since CSS variables can be redefined by later rules, the browser does not know the local scope when it is constructing the style sheet. Therefore it cannot do things that you might be used to in javascript such as check if the variable is undefined, converting it, or doing any other type of manipulation. Instead, it's based on this inheritance model. 

CSS nodes have computed properties that are inherited from the parent node. So when it encounters a CSS variable the browser 
